...
import { notificationEvents } from '../../interfaces/notifications-events.interface';
import { CorreoApiSendinblueService } from '../../services/correo-api-sendinblue.service';

@Component({
  selector: 'app-notifications-events',
  templateUrl: './notifications-events.component.html',
  styleUrls: ['./notifications-events.component.css']
})
export class NotificationsEventsComponent implements OnInit {
  ...
  campaigns : Campaigns = { // este arreglo es para realiza el registro de campañas
    ...
    name_campaigns: 'Campaña Notificacion de eventos',
    id_plantilla: 0,
    business: '',
  ...
  getDataCliente() {
    ...
    if(this.empresass[0] === this.selectedEmpresas[0]) { // FUNERARIA LAS FLORES
     ...
     this.service.getDataClienteFunerariaNotifications()
      .subscribe(
        resp => {
      ...
      this.service.getDataClienteNotifications()
      .subscribe(
        resp => {
          this.datosClientes = resp;
          //console.log(this.datosCliente)
     ...
     // alert("eligio Cementerio")
    ...
    }else if (this.empresass[2] === this.selectedEmpresas[0]) { // ALTAMIRA

      this.service.getDataClienteAltamiraNotifications()
      ...
      this.service.getDataClienteIndelpaNotifications()
      .subscribe(
        resp => {
          this.datosClientes = resp;
          //console.log(this.datosCliente)
